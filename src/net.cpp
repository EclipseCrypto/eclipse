...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "EclipseCrypto " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. EclipseCrypto is probably already running."), addrBind.ToString().c_str());
        else
...
void StartNode(boost::thread_group& threadGroup)
{
    ...
    LogPrintf("StartNode(): pnodeLocalHost addr: %s\n", pnodeLocalHost->addr.ToString().c_str());
    ...
    if (!GetBoolArg("-onionseed", true))
        ...
        if (!NewThread(ThreadOnionSeed, NULL))
              LogPrintf("Error: could not start .onion seeding\n");
