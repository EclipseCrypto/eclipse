...
void EclipseGUI::pageLoaded(bool ok)
{
...
void EclipseGUI::addJavascriptObjects()
{
    documentFrame->addToJavaScriptWindowObject("bridge", bridge);
}

void EclipseGUI::urlClicked(const QUrl & link)
{
...
void EclipseGUI::createActions()
{
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About EclipseCrypto"), this);
    aboutAction->setToolTip(tr("Show information about EclipseCrypto"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for EclipseCrypto"));
...
void EclipseGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("EclipseCrypto") + " - " + tr("Wallet") + ", " + sMode);
        };
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/eclipse_testnet"));
            setWindowIcon(QIcon(":icons/eclipse_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/eclipse_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("EclipseCrypto client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/eclipse_testnet"));
...
void EclipseGUI::setWalletModel(WalletModel *walletModel)
{
...
void EclipseGUI::setMessageModel(MessageModel *messageModel)
{
...
void EclipseGUI::createTrayIcon()
{
    QMenu *trayIconMenu;
#ifndef Q_OS_MAC
    ...
    trayIcon->setToolTip(tr("Eclipse client"));
    trayIcon->setIcon(QIcon(":/icons/eclipse"));
...
#ifndef Q_OS_MAC
void EclipseGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void EclipseGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("data-title", tr("%n active connection(s) to EclipseCrypto network", "", count));
}
...
void EclipseGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void EclipseGUI::changeEvent(QEvent *e)
{
...
void EclipseGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void EclipseGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          "which goes to the nodes that process your transaction and helps to support the network.  "
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::EC, nFeeRequired));
...
void EclipseGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
...
void EclipseGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void EclipseGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void EclipseGUI::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
}

void EclipseGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid EclipseCrypto address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void EclipseGUI::handleURI(QString strURI)
{
...
void EclipseGUI::setEncryptionStatus(int status)
{
...
void EclipseGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void EclipseGUI::changePassphrase()
{
...
void EclipseGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void EclipseGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void EclipseGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void EclipseGUI::updateStakingIcon()
{
...
void EclipseGUI::detectShutdown()
{
