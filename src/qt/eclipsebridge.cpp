...
#include "eclipsebridge.moc"

EclipseBridge::EclipseBridge(EclipseGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
void EclipseBridge::setClientModel()
{
...
// This is just a hook, we won't really be setting the model...
void EclipseBridge::setWalletModel()
{
...
// This is just a hook, we won't really be setting the model...
void EclipseBridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void EclipseBridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void EclipseBridge::paste()
{
    emitPaste(QApplication::clipboard()->text());
}

// Options
void EclipseBridge::populateOptions()
{
...
// Transactions
bool EclipseBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void EclipseBridge::clearRecipients()
{
    recipients.clear();
}

bool EclipseBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    ...
    {
        int inputType; // 0 EC, 1 Eclipse
        switch(rcp.txnTypeInd)
        {
            case TXT_EC_TO_EC:
#if QT_VERSION < 0x050000
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::EC, rcp.amount), Qt::escape(rcp.label), rcp.address));
#else
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::EC, rcp.amount), rcp.label.toHtmlEscaped(), rcp.address));
#endif
                inputType = 0;
                break;
            case TXT_EC_TO_ANON:
#if QT_VERSION < 0x050000
                formatted.append(tr("<b>%1</b> to ECLIPSE %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::EC, rcp.amount), Qt::escape(rcp.label), rcp.address));
#else
                formatted.append(tr("<b>%1</b> to ECLIPSE %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::EC, rcp.amount), rcp.label.toHtmlEscaped(), rcp.address));
#endif
                inputType = 0;
                nAnonOutputs++;
                break;
...
#if QT_VERSION < 0x050000
                formatted.append(tr("<b>%1</b>, ring size %2 to ECLIPSE %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::EC, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
#else
                formatted.append(tr("<b>%1</b>, ring size %2 to ECLIPSE %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::EC, rcp.amount), QString::number(rcp.nRingSize), rcp.label.toHtmlEscaped(), rcp.address));
#endif
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_EC:
#if QT_VERSION < 0x050000
                formatted.append(tr("<b>%1</b>, ring size %2 to EC %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::EC, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
#else
                formatted.append(tr("<b>%1</b>, ring size %2 to EC %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::EC, rcp.amount), QString::number(rcp.nRingSize), rcp.label.toHtmlEscaped(), rcp.address));
#endif
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::EC, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedsEclipseBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your eclipse balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::EC, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void EclipseBridge::openCoinControl()
{
...
void EclipseBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void EclipseBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap EclipseBridge::listAnonOutputs()
{
...
void EclipseBridge::populateTransactionTable()
{
...
void EclipseBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void EclipseBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString EclipseBridge::transactionDetails(QString txid)
{
...
// Addresses
void EclipseBridge::populateAddressTable()
{
...
void EclipseBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void EclipseBridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString EclipseBridge::newAddress(QString addressLabel, int addressType, QString address, bool send)
{
...
QString EclipseBridge::lastAddressError()
{
    QString sError;
...
QString EclipseBridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void EclipseBridge::updateAddressLabel(QString address, QString label)
{
...
bool EclipseBridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool EclipseBridge::deleteAddress(QString address)
{
    return addressModel->atm->removeRow(addressModel->atm->lookupAddress(address));
}

// Messages
void EclipseBridge::appendMessages(QString messages, bool reset)
{
    emitMessages("[" + messages + "]", reset);
}

void EclipseBridge::appendMessage(int row)
{
...
void EclipseBridge::populateMessageTable()
{
...
void EclipseBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool EclipseBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool EclipseBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString EclipseBridge::getPubKey(QString address, QString label)
{
    if(!label.isEmpty())
...
bool EclipseBridge::setPubKey(QString address, QString pubkey)
{
...
bool EclipseBridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
    WalletModel::UnlockContext ctx(window->walletModel->requestUnlock());
...
void EclipseBridge::connectSignals()
{
...
QVariantMap EclipseBridge::userAction(QVariantMap action)
{
...
// Blocks
QVariantMap EclipseBridge::listLatestBlocks()
{
...
QVariantMap EclipseBridge::findBlock(QString searchID)
{
...
QVariantMap EclipseBridge::blockDetails(QString blkHash)
{
...
QVariantMap EclipseBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap EclipseBridge::txnDetails(QString blkHash, QString txnHash)
{
...
QVariantMap EclipseBridge::signMessage(QString address, QString message)
{
    QVariantMap result;
...
QVariantMap EclipseBridge::verifyMessage(QString address, QString message, QString signature)
{
    QVariantMap result;
...
QVariantMap EclipseBridge::getNewMnemonic(QString password, QString language)
{
    QVariantMap result;
...
QVariantMap EclipseBridge::importFromMnemonic(QString inMnemonic, QString inPassword, QString inLabel, bool fBip44, int64_t nCreateTime)
{
...
QVariantMap EclipseBridge::extKeyAccList() {
    QVariantMap result;
...
QVariantMap EclipseBridge::extKeyList() {
    QVariantMap result;
...
QVariantMap EclipseBridge::extKeyImport(QString inKey, QString inLabel, bool fBip44, int64_t nCreateTime)
{
    QVariantMap result;
    ...
    if (eKey58.Set58(sInKey.c_str()) == 0)
    {
        ...
        {
            result.insert("error_msg", "Import failed - Key must begin with Eclipsecoin prefix.");
            return result;
        }
...
QVariantMap EclipseBridge::extKeySetDefault(QString extKeyID)
{
    QVariantMap result;
...
QVariantMap EclipseBridge::extKeySetMaster(QString extKeyID)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
    if (extKeyID.length() == 0)
...
QVariantMap EclipseBridge::extKeySetActive(QString extKeyID, QString isActive)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
