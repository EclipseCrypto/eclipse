...
var unit = {
    ...
    setType: function(type) {
        ...
        switch(type) {
            ...
            case 2:
                this.name = "uEC",
                this.display = "&micro;EC";
                break;

            case 3:
                this.name    = "sEC",
                this.display = "Eclipseshi";
                break;

            default:
                this.name = this.display = "EC";
        }
...
var overviewPage = {
    init: function() {
        ...
        this.eclipseBal = $(".eclipse_balance"),
        ...
        // Announcement feed
        $.ajax({
            url:"http://ajax.googleapis.com/ajax/services/feed/load?v=2.0\&q=http://eclipsehangout.com/category/18.rss",
            dataType: 'jsonp'
    ...
    updateBalance: function(balance, eclipseBal, stake, unconfirmed, immature) {
        if(balance == undefined)
            balance     = this.balance    .data("orig"),
            eclipseBal   = this.eclipseBal  .data("orig"),
            ...
            this.eclipseBal  .data("orig", eclipseBal),
        ...
        this.formatValue("eclipseBal",   eclipseBal);
    ...
    $("#recipients").append((
        ...
        +  '<div id="recipient[count]" class="recipient"> \
            <div class="flex-right"> \
                ...
                <input id="pay_to[count]" class="pay_to input_box" data-title="The address to send the payment to  (e.g. EcH3baudDZxibNgouuPdRVLJT3kbYTvKxr)" placeholder="Enter a Eclipse address (e.g. EcH3baudDZxibNgouuPdRVLJT3kbYTvKxr)" maxlength="128" oninput="base58.check(this);" onchange="$(\'#label[count]\').val(bridge.getAddressLabel(this.value));"/> \
            ...
            <div class="flex-left"> \
                ...
                <select id="unit[count]" class="unit button is-inverse has-fixed-icon"  style="margin-left:10px; height:43px; width:100px;" onchange="unit.format(\'#amount[count]\', $(this).val());"> \
                    <option value="0" title="Eclipse"                    ' + (unit.type == 0 ? "selected" : "") + '>EC</option> \
                    <option value="1" title="Milli-Eclipse (1 / 1000)"   ' + (unit.type == 1 ? "selected" : "") + '>mEC</option> \
                    <option value="2" title="Micro-Eclipse (1 / 1000000)"' + (unit.type == 2 ? "selected" : "") + '>&micro;EC</option> \
                    <option value="3" title="Eclipseshi (1 / 100000000)" ' + (unit.type == 3 ? "selected" : "") + '>Eclipseshi</option> \
                </select> \
            </div> \
...
function changeTxnType()
{
    ...
    {
        $("#tx_ringsize,#suggest_ring_size,#spend_eclipse").hide();
