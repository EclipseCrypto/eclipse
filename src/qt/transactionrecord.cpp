...
QString TransactionRecord::getTypeLabel(const int &type)
{
    ...
    case RecvWithAddress:
        return EclipseGUI::tr("Received with");
    case RecvFromOther:
        return EclipseGUI::tr("Received from");
    case SendToAddress:
    case SendToOther:
        return EclipseGUI::tr("Sent to");
    case SendToSelf:
        return EclipseGUI::tr("Payment to yourself");
    case Generated:
        return EclipseGUI::tr("Mined");
    case RecvEclipse:
        return EclipseGUI::tr("Received eclipse");
    case SendEclipse:
        return EclipseGUI::tr("Sent eclipse");
    case Other:
        return EclipseGUI::tr("Other");
    default:
        return "";
    }
}
...
QString TransactionRecord::getTypeShort(const int &type)
{
    ...
    case TransactionRecord::RecvEclipse:
        return "input";
    case TransactionRecord::SendToAddress:
    case TransactionRecord::SendToOther:
    case TransactionRecord::SendEclipse:
        return "output";
    default:
        return "inout";
    }
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    int64_t nCredEC, nCredEclipse;
    wtx.GetCredit(nCredEC, nCredEclipse, true);
    int64_t nCredit = nCredEC + nCredEclipse;
    ...
    {
        if (nNet > 0 && nCredEclipse > 0)
        {
            // -- credit
            TransactionRecord sub(hash, nTime, TransactionRecord::RecvEclipse, "", "", nNet, 0);
        ...
        {
            // -- debit
            TransactionRecord sub(hash, nTime, TransactionRecord::SendEclipse, "", "", nNet, 0);
        ...
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
        {
                ...
                && txout.IsAnonOutput())
            {
                ...
                if (wallet->HaveKey(ckidD))
                {
                    ...
                    sub.type = TransactionRecord::RecvEclipse;
    ...
    } else
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
                ...
                && txin.IsAnonInput())
            {
                ...
                {
                    fAllFromMe = false;
                    break; // display as send/recv eclipse
                };
                continue;
            };
        ...
        if (fAllFromMe)
        {
            ...
            for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
            {
                    ...
                    && txout.IsAnonOutput())
                {
                    ...
                    sub.type = TransactionRecord::SendEclipse;
                    sub.address = CBitcoinAddress(ckidD).ToString();
