...
public:
    ...
    int64_t GetEclipseBalance() const;
    ...
    int64_t GetEclipseDebit(const CTxIn& txin) const;
    int64_t GetEclipseCredit(const CTxOut& txout) const;
    ...
    int64_t GetCredit(const CTransaction& tx) const
    {
        ...
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nCredit += GetEclipseCredit(txout);
            } else
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nEC, int64_t& nEclipse) const
    {
        nEC = 0;
        nEclipse = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nEclipse += GetEclipseCredit(txout);
            } else
                nEC += GetCredit(txout);
            if (!MoneyRange(nEC)
                || !MoneyRange(nEclipse))
...
public:
    ...
    mutable bool fAvailableEclipseCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailableEclipseCreditCached;
    
    mutable int64_t nCredECCached;
    mutable int64_t nCredEclipseCached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailableEclipseCreditCached = false;
        
        nCredECCached = 0;
        nCredEclipseCached = 0;
        ...
        nAvailableEclipseCreditCached = 0;
        nChangeCached = 0;
        nOrderPos = -1;
    }
    ...
    bool GetCredit(int64_t& nCredEC, int64_t& nCredEclipse, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredEC = nCredEclipse = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredECCached = 0;
            nCredEclipseCached = 0;
            pwallet->GetCredit(*this, nCredECCached, nCredEclipseCached);
            fCreditSplitCached = true;
        };
        
        nCredEC = nCredECCached;
        nCredEclipse = nCredEclipseCached;
        return true;
    }
    ...
    int64_t GetAvailableEclipseCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailableEclipseCreditCached)
            return nAvailableEclipseCreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailableEclipseCredit() : value out of range");
            };
        };
        
        nAvailableEclipseCreditCached = nCredit;
        fAvailableEclipseCreditCached = true;
        return nCredit;
    };
