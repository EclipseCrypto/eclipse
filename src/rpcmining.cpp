...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "EclipseCrypto is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "EclipseCrypto is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "EclipseCrypto is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "EclipseCrypto is downloading blocks...");
    ...
    if (params.size() == 0)
    {
        ...
        if ((pindexPrev != pindexBest) ||
            ...
            (pblock->GetBlockTime() >= FutureDrift((int64_t)pblock->vtx[0].nTime, pindexPrev->nHeight + 1) + 15))
        {
            if (pindexPrev != pindexBest)
            {
                // Deallocate old blocks since they're obsolete now
