...
static int hashToEC(const uint8_t *p, uint32_t len, BIGNUM *bnTmp, EC_POINT *ptRet)
{
    // EC_COMPRESSED_SIZE == 32 (types.h)
    uint256 pkHash = Hash(p, p + len);

    unsigned char* pcHash = static_cast<unsigned char*>(static_cast<void*>(&pkHash));
    ...
    if (!EC_POINT_set_affine_coordinates_GFp(ecGrp, ptRet, bn_x, bn_y, bnCtx))
        return 1;
